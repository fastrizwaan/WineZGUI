...
class RadioButtonWindow(Gtk.Window):
    def __init__(self):
        ...
        label = Gtk.Label(label="Wine GUI using Zenity...")
    ...
    def on_ok_clicked(self, button):
        if self.selected_button:
            label = self.selected_button.get_label()
            print(f"Executing command for '{label}'")
            if label == "Open Exe...":
                result = subprocess.run(["./hello.sh"], capture_output=True, text=True)
                print(result.stdout)
            # Add more commands for other radio buttons if needed
        self.close()
